...
(2026-07-20 추가) 백엔드 작업 중 "지출결의서인데 금액을 담을 컬럼이 없다"는
문제를 뒤늦게 발견해 AMOUNT 컬럼을 추가했습니다. LEAVE_START_DATE와 같은
...
### 2-13-1. APPROVAL_FILE — 결재 첨부파일 (2026-07-20 추가)

```
지출결의서(영수증 등 증빙)·프로젝트품의서(사양서 등)는 첨부파일이 필요한데,
기존 설계엔 이걸 담을 곳이 없었습니다. 자료실의 ARCHIVE_FILE과 완전히
같은 구조로 새로 만들었습니다 - 한 결재 문서(1)에 첨부파일 여러 개(N)가
붙는 1:N 관계이기 때문입니다.

연차휴가신청서는 증빙 서류 개념이 없어 이 테이블을 안 씁니다(첨부파일
UI 자체가 화면에 안 뜸). 파일 실제 저장 방식(UUID 파일명 등)도 자료실과
동일한 규칙을 그대로 따릅니다.
```

| 컬럼명 | 타입 | 제약조건 | 설명 |
| --- | --- | --- | --- |
| FILE_ID | INT | PK, AUTO_INCREMENT | 첨부파일 고유 번호 |
| APPROVAL_ID | INT | FK(APPROVAL), NOT NULL | 소속 결재 문서 |
| FILE_NAME | VARCHAR(300) | NOT NULL | 원본 파일명 (다운로드 시 이 이름으로 내려줌) |
| FILE_PATH | VARCHAR(500) | NOT NULL | 실제 저장 경로 (UUID 기반 파일명) |
...
| APPROVAL : APPROVAL_FILE | 1 : N | 한 문서에 여러 첨부파일이 등록됨 (2026-07-20 추가) |
